...
namespace Accord.Neuro.Learning
{
    ...
    public class ContrastiveDivergenceLearning : IUnsupervisedLearning, IDisposable
    {
        ...
        private double sparsePanalty = 0.99;
            ...
            Parallel.For(0, input.Length,
...
#endif
                {
                    ...
                    for (int j = 0; j < hidden.Neurons.Length; j++)
                    {
                     ...
                     //   probability[j] = hidden.Neurons[j].Generate(observation); 
                    ...
                    for (int j = 0; j < visible.Neurons.Length; j++)
                      ...
                      //  reconstruction[j] = visible.Neurons[j].Generate(activations);
                ...
                (partial) =>
                {
                    lock (lockObj)
                    {
                        ...
                        double[] oldhidden=new double[hiddenBiasGradient.Length] ;
                        Array.Copy(hiddenBiasGradient,oldhidden,hiddenBiasGradient.Length);
                        for (int i = 0; i < hiddenBiasGradient.Length; i++)
                            hiddenBiasGradient[i] += partial.HiddenBiasGradient[i];
                        // Mu Li modified for sparsepanalty on 2013/03/04
                        double currentsum = (1 - sparsePanalty) * hiddenBiasGradient.Sum() / (double)hiddenBiasGradient.Length;
                        for (int i = 0; i < hiddenBiasGradient.Length; i++)
                            hiddenBiasGradient[i] = currentsum + sparsePanalty*oldhidden[i];
